...
## 今日产出
- [x] 08_structured_output.py（07 已被 radix_attention 占用，改用 08）
- [x] 简单/复杂/对抗三组数据（无约束 0% → 约束 100%；enum 守规矩；对抗"写诗"仍合法 JSON）
- [x] 结构化输出使用笔记（含 FSM 约束解码原理图解）

> 完成于 2026-06-13。完整笔记：[[SGLang结构化输出实验-06-13]]
> 核心：约束解码在 logits 层把非法 token 置 -∞，FSM 编译开销 ~0.28s（首次 1.86s vs 后续 1.57s）。
