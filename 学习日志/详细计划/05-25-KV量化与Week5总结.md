...
## 今日产出
- [x] KV 量化对比（fp16 246,496 token vs fp8 493,008 token，精确翻倍）→ [[进阶特性总结]]
- [x] 并发提升换算（60.18x → 120.36x，2.0x）
- [x] 进阶特性总结.md（投机/结构化/量化三大特性汇总）

> 完成于 2026-06-14。★FP8 KV cache 实测精确翻倍(并发 60→120x)。
> Week5 三特性都是可选插件，按场景启用，与框架选型正交。
