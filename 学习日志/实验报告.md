...
实测最高吞吐换算每百万 token 成本（[[成本对比-06-14]]）：

| GPU | vLLM | SGLang |
|---|---:|---:|
| A10 (¥5/h) | ¥0.96/M | ¥1.04/M |
| A100 (¥18/h) | ¥3.47/M | ¥3.73/M |

- 无共享前缀：成本差异 = 吞吐差异（8%），同量级。
- **高复用：SGLang 成本可减半**（吞吐 2.1x）——统一 system prompt 的客服系统，那段 KV 算一次全局共享。

> **解读**：成本是性能的现实投影。SGLang 在高复用场景的吞吐优势直接转化为成本优势（减半），这是最有说服力的选型理由。
...
1. **KV Cache 管理（块 vs token）是两框架最大差异**——并行策略都是 Megatron 底座，不区分。
2. **复用率是分水岭**——无/低复用持平（vLLM 略优 8%），高复用 SGLang 吞吐 2.1x、成本减半。
3. **延迟、结构化输出、量化**两框架相当，不是区分点。

### 选型表

| 场景 | 推荐 | 数据依据 |
|---|---|---|
| 通用/低复用批量推理 | **vLLM** | 吞吐略优 8% + 生态成熟 |
| 高复用（统一 prompt/RAG/Agent） | **SGLang** | 吞吐 2.1x、成本减半 |
| 多轮对话 | 两者皆可 | 都有前缀缓存 |
| 结构化输出 | 两者皆可 | 都 100% 合法 |
| 显存紧张/长上下文 | 任意 + 量化 | AWQ 省 64% / FP8 KV 翻倍 |
| 格式化输出 | 任意 + ngram 投机 | 重复内容 4.5x |

### 局限声明

基于 RTX 3060 12GB / Qwen2.5-1.5B / vLLM 0.22.1 / SGLang 0.5.13。**趋势可外推，具体数字（2.1x、8%）需在目标硬件（大模型/多卡）实测验证**。社区数据（H100 上 29%~6.4x）佐证了趋势一致性。
...
| vLLM | 0.22.1（基础环境） |
| SGLang | 0.5.13（venv-sglang） |
| 数据 | bench_results/*.csv（3 轮中位数） |

### B. 参考资料

- PagedAttention 论文 arXiv:2309.06180
- SGLang/RadixAttention 论文 arXiv:2312.07104
- 社区交叉验证见 [[社区数据交叉验证]]
